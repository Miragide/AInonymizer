...
```bash
# Run tests (no external dependencies needed beyond the project's own)
python test_siegfried.py

# Run the anonymizer on all PDF/DOCX/MD files in the current directory
python siegfried.py
...
**Siegfried** is a local, rule-based pseudonymizer for French legal documents. It processes PDF, DOCX, and Markdown files through a 4-pass pipeline and outputs anonymized Markdown + JSON mapping files.
...
2. **Post-processing** (`juritools.py`) — reclassifies dates (procedural vs. birth), propagates multi-occurrences, finds orthographic variants (Levenshtein ≤ 2), trims particles, cross-references physical/legal persons
...
- `siegfried.py` scans only the script's own directory for input files (not recursively), excluding `output/`.
...
| File | Role |
|---|---|
| `siegfried.py` | Entry point: file discovery, orchestration, Markdown + JSON output |
...
2. Export the new function from `regex_fr.py` and call it in `anonymizer.py:analyze()`
3. Add the priority score to `_PRIORITY` in `anonymizer.py`
4. Add a test case in `test_siegfried.py`
